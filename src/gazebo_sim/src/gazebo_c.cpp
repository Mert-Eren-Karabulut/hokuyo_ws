...
std::string urdf_file_path="/home/mali/turtlebot_ws/src/gazebo_sim/urdf/robot3.urdf.xacro";
...

{
    ...
    for(int i = 0; i < M; i++){
        ...
        float x = allObstacles.at(i).x; float y = allObstacles.at(i).y; float z = 0;

        float dim = 0.05;    float mass = 0.05;
        float r = allObstacles.at(i).rho; float h = allObstacles.at(i).h;

        cout << "Adding obstacle  -->" << name <<  endl;

        spawner.spawnPrimitive(name, false, "world", x, y, z, 0, 0, 0, 1, r, h, dim, mass);
    }

    std::cout << "All obstacles spawned \n";
