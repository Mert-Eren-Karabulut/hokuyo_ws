...
float start_angle = -2.094f;  // Laser start angle
...
// Joint state callback from Arduino with TF-based pointcloud generation
void jointStateCallback(const sensor_msgs::JointState::ConstPtr &msg) {
    ...
    if (msg->position.size() >= 2) {
        ...
        if (dt.count() > 0.001) {  // Minimum 1ms between samples to avoid division by zero
    ...
    // Update parametric targets if scanning is active
    if (scanning_active) {
        ...
        // Process laser scan into voxel grid when there's significant movement
        if (fabs(tiltangle - lastangle) >= (PI)/180 || fabs(panangle - lastpan) >= (PI)/180) {
            if (!laser_scan.empty() && tf_buffer) {
                ...
                for (int i = 0; i < scanSize; i++) {
                    ...
                    try {
                        ...
                        point_hokuyo.header.frame_id = "laser";
                        ...
                        // Calculate distance from sensor for quality tracking
